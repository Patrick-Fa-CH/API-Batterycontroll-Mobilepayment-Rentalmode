...
def set_fm_mos_charging_off(header,battery_number: str):
    
    DEMO_MODE = os.environ.get("DEMO_MODE", "false").lower() == "true"
    if DEMO_MODE == "true":
        print("-----DEMO: Battery charge deactivated through API-----", battery_number)
        return True 
    ...
    while attempt < max_retries and (time.time() - start_time) < max_total_time:
        ...
        try:
            ...
            if data.get("response") is True:
                print("-----Battery charge deactivated through API-----", battery_number)
                return True
