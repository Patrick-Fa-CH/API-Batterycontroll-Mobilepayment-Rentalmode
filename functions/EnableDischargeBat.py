...
def set_fm_mos_discharging_on(header,battery_number: str):

    DEMO_MODE = os.environ.get("DEMO_MODE", "false").lower() == "true"
    if DEMO_MODE == "true":
        print("-----DEMO: Battery discharge activated through API-----", battery_number)
        return True 
    ...
    while attempt < max_retries and (time.time() - start_time) < max_total_time:
        ...
        try:
            response = requests.post(BATTERY_API_COMMAND_URL)
            response.raise_for_status() #check if request was successful, no error code
            data = response.json()

            if data.get("response") is True:
                print("-----Battery discharge activated through API-----", battery_number)
                return True
